═══════════════════════════════════════════════════════════════════════════════
 AUDIO TITLE — rename the generated audio in NLM Studio to exactly this:
    ...
    aristoteles_07_l06_c10_cena01_fisica

 (Final filename on disk: aristoteles_07_l06_c10_cena01_fisica.m4a)
═══════════════════════════════════════════════════════════════════════════════
...
- **Audio Position in This Work:** This is audio 56 of 75 for *Physics*.
...
1. **Título Principal:** "Guia de Estudos: Física — BOOK VI, Capítulo Part 10 (Cena 1/1)"
...
The sources uploaded to NotebookLM contain the full text of *Physics*. For this specific guide, focus EXCLUSIVELY on the passage from BOOK VI, Part 10, bounded by:

**Starts at:** "Our next point is that that which is without parts cannot be in motion except accidentally: i.e. it can be in motion only in so far as the body or the magnitude is in motion and the partless is in mot"
**Ends at:** "If it is to be one process, no motion can be infinite in respect of the time that it occupies, with the single exception of rotatory locomotion."
