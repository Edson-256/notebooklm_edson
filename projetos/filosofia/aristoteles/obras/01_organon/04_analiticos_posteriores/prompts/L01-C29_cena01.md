═══════════════════════════════════════════════════════════════════════════════
 AUDIO TITLE — rename the generated audio in NLM Studio to exactly this:
    ...
    aristoteles_04_l01_c29_cena01_analiticos_posteriores

 (Final filename on disk: aristoteles_04_l01_c29_cena01_analiticos_posteriores.m4a)
═══════════════════════════════════════════════════════════════════════════════
...
- **Passage Identifier:** BOOK I, Part 29 (sub-cena 1 of 1).
- **Audio Position in This Work:** This is audio 29 of 53 for *Posterior Analytics*.
...
1. **Título Principal:** "Guia de Estudos: Analíticos Posteriores — BOOK I, Capítulo Part 29 (Cena 1/1)"
...
The sources uploaded to NotebookLM contain the full text of *Posterior Analytics*. For this specific guide, focus EXCLUSIVELY on the passage from BOOK I, Part 29, bounded by:

**Starts at:** "One can have several demonstrations of the same connexion not only by taking from the same series of predication middles which are other than the immediately cohering term e.g. by taking C, D, and F s"
**Ends at:** "not in the same series-yet not so that neither of these middles is predicable of the other, for they must both be attributable to some one subject."
