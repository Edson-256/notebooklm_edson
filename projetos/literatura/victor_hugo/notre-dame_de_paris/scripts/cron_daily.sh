#!/bin/bash
# Cron wrapper: Notre Dame de Paris (français) — 3 áudios/dia (limite free).
...
LOG="$LOG_DIR/notredame_cron_${TS}.log"
TAG="Notre Dame"
PROFILE="frances"
...
notify() {
  ...
  if command -v terminal-notifier >/dev/null 2>&1; then
    terminal-notifier -title "$title" -message "$msg" -sound "$sound" \
      -group "notredame-cron" >/dev/null 2>&1
  else
...
# O runner grava logs/notredame_lastrun.json (criação+download+transferência);
...
bash "$REPO_DIR/scripts/git_state_commit.sh" \
  "projetos/literatura/victor_hugo/notre-dame_de_paris/audios/metadata.json" "notredame" >/dev/null 2>&1 || true
